(function () {
  ...
  window.__BRAND__ = {
    ...
    // Clave pública VAPID (RFC 8292) para las alertas push (ver alertas.js y
    // src/index.js). No es secreta: es la misma que usa el servidor al
    // firmar cada envío, así que tiene que coincidir exactamente con
    // VAPID_PUBLIC_KEY en src/index.js o el navegador rechazará la
    // suscripción.
    vapidPublicKey: "BMEERguBY5t2hfTHsgfBNAWu8tqcnTSz3qsoC5-_J77thziY1aiWNaH11eWCrNxfTQWQP4KRH8axEiynKRoNCiI",
    ...
    faqs: [
      ...
      },
      {
        ...
        r: "El stop loss se coloca a una distancia fija de 100 pips desde el borde de la zona de entrada. Los take profit se calculan pensando en el scalping: se priorizan los niveles de liquidez más próximos en la dirección de la operación (el máximo o mínimo del propio movimiento impulsivo, o el PDH/PDL si aún no se han barrido) y, si no hay liquidez cercana, se usan extensiones de Fibonacci del rango; en todos los casos se acotan entre 1,5 y 4 veces el riesgo para que los objetivos sean realistas. Es una forma de calcularlos entre varias posibles, no la única ni una fórmula infalible."
      },
      {
